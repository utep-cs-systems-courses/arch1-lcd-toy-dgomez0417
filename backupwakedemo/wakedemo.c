...
void main()
{
  ...
  while (1) {                   /* forever */
    ...
    if (redrawScreen) {
      redrawScreen = 0;
      drawString8x12(WordCol,20, "Reshape Game", COLOR_BLACK, COLOR_BLACK);
      drawString8x12(nextWordCol,20, "Reshape Game", fontFgColor, COLOR_BLACK);
      WordCol = nextWordCol;
    }
...
/*void switchesDiamond(){
  ...
  str[0] = (switches & (1<<0)) ? '-' : '0'+0;
  str[1] = (switches & (1<<1)) ? '-' : '0'+1; 
  str[2] = (switches & (1<<2)) ? '-' : '0'+2;
  str[3] = (switches & (1<<3)) ? '-' : '0'+3;
  if(str[0]){
    makeDiamond();
  }
  else{
